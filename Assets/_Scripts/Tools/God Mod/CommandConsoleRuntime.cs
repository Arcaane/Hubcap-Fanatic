...
namespace HubcapManager {
    public class CommandConsoleRuntime : SingletonUpdatesHandler<CommandConsoleRuntime>, IUpdate {
        private ToolsInput inputs;
        [SerializeField] private TMP_InputField inputField = null;
        [SerializeField] private TextMeshProUGUI text = null;
        [SerializeField] private GameObject objectChild = null;
        public GameObject ObjectChild => objectChild;
        [Space] [SerializeField] private Transform textTransformArea = null;
        [SerializeField] private GameObject textPrefab = null;
        [Space] [SerializeField] private Transform parameterTransformArea = null;
        ...
        private List<GameObject> methodList = new();
        ...
        protected override void Start() {
            base.Start();
            CommandConsole HELP = new CommandConsole(
                "help", 
            ...
            SceneManager.sceneLoaded += UpdateCommandOnSceneSwitch;
        }

        private void UpdateCommandOnSceneSwitch(Scene arg0, LoadSceneMode arg1) {
            foreach (object obj in commandList.ToList()) {
                if (obj is not CommandConsole command) continue;
                if (command.KeepOnSceneLoaded) continue;
                commandList.Remove(obj);
            }
        }

        public void UpdateTick() {
            if (inputField == null || !inputField.IsActive()) return;

            if (Input.GetKeyDown(KeyCode.Return)) CheckEndText();
            if (Input.GetKey(KeyCode.LeftShift) && Input.GetKeyDown(KeyCode.Backspace)) RemoveText();
        ...
        private void CreateParameterPrefab(string textToShow, int textSize, int actualTextWriteLength) {
            ...
            trans.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, textSize * 12, textToShow.Length * 20f);
    ...
    public class CommandConsole : CommandConsoleBase {
        ...
        private bool keepOnSceneLoaded = false;
        public bool KeepOnSceneLoaded => keepOnSceneLoaded;

        public CommandConsole(string commandName, string descriptionCommand, List<CommandClass> commandsClassType, Action<List<string>> actionEvent, bool keepOnSceneLoaded) : base(commandName, descriptionCommand, commandsClassType) {
            this.actionEvent = actionEvent;
            this.keepOnSceneLoaded = keepOnSceneLoaded;
        }
