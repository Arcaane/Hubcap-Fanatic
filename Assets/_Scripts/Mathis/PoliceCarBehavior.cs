...
public class PoliceCarBehavior : MonoBehaviour
{
    [Header("REFERENCES")] 
    ...
    private void Update()
    {
        ...
        rotationValue = -Mathf.Clamp(Vector2.SignedAngle(new Vector2(transform.forward.x,transform.forward.z),
            new Vector2(target.position.x, target.position.z) -
            new Vector2(transform.position.x, transform.position.z)) / 10,-1,1);
        
        Debug.DrawRay(transform.position + Vector3.up *3,new Vector2(transform.forward.x,transform.forward.z),Color.red);
        Debug.DrawRay(transform.position + Vector3.up *3,new Vector2(target.position.x, target.position.z) - new Vector2(transform.position.x, transform.position.z),Color.cyan);
