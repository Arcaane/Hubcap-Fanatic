...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float Step, out float SceneDepth, out float3 Color, out float Edges, out float3 VertexColors)
{
    ...
    Color = SAMPLE_TEXTURE2D(_CameraOpaqueTexture, sampler_CameraOpaqueTexture, UV);
    ...
    edgeVertexColor = edgeVertexColor > 0.01f ? 1 : 0;
